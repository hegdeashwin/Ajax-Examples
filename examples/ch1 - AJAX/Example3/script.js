/***
 * Example 1: Understanding XHR Output.
...
function initAll() {
	document.getElementById("requestText").onclick = function() {
		textRequest = true; // Shows we want text request.
		makeRequest();

		/***
		 * JS tells the browser that the link user clicked on has been handled by JS
		 */
		return false;
	}
	document.getElementById("requestXML").onclick = function() {
		textRequest = false; // Shows we don't want request.
		makeRequest();
		return false;
	}
}
